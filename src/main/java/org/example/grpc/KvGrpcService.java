...
public class KvGrpcService extends KVServiceGrpc.KVServiceImplBase {
        ...
        try {
            ...
            if (result.isFound()) {
                ...
                if (result.getValue() != null) {
                    builder.setValue(BytesValue.of(ByteString.copyFrom(result.getValue())));
                }
            }
    ...
    public void range(RangeRequest req, StreamObserver<KeyValuePair> resp) {
        try {
            List<KvResult> results = repo.range(req.getKeySince(), req.getKeyTo());

            for (KvResult r : results) {
                KeyValuePair.Builder pair = KeyValuePair.newBuilder().setKey(r.getKey());
                if (r.getValue() != null) {
                    pair.setValue(BytesValue.of(ByteString.copyFrom(r.getValue())));
                }
